...
export default function HonoursAdminPage() {
    const [achievements, setAchievements] = useState([]);
    const [loading, setLoading] = useState(true);
    const [editingId, setEditingId] = useState(null);
    const [formData, setFormData] = useState({ title: '', designation: '', image_url: '', link: '/photo-gallery/' });
    ...
    const handleDelete = async (id) => {
        ...
        try {
            const res = await fetch(`/api/honours-milestones?id=${id}`, { method: 'DELETE' });
            if (res.ok) {
                setAchievements(achievements.filter(f => f.id !== id));
            }
        } catch (error) {
    ...
    const resetForm = () => {
        setEditingId(null);
        setFormData({ title: '', designation: '', image_url: '', link: '/photo-gallery/' });
        setIsAdding(false);
    };
    ...
    return (
        <div className="min-h-screen bg-gray-50/50 pb-20">
            <div className="bg-white border-b px-6 py-4 sticky top-0 z-10 shadow-sm flex justify-between items-center">
                <div>
                    <h1 className="text-2xl font-bold text-gray-800">Honours & Milestones</h1>
                    <p className="text-sm text-gray-500">Manage honours carousel.</p>
                </div>
                {!isAdding && (
                    <button 
            ...
            <div className="p-6 max-w-6xl mx-auto space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {loading ? <div className="col-span-full text-center py-10 text-gray-400">Loading...</div> : achievements.map(item => (
                        <div key={item.id} className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden group hover:shadow-md transition-shadow">
                            <div className="h-56 relative bg-gray-100">
                                ...
                                <div className="absolute top-2 right-2 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity bg-white/90 p-1.5 rounded-lg shadow-sm">
                                    <button onClick={() => startEdit(item)} className="text-blue-600 hover:bg-blue-50 p-1 rounded"><FaEdit /></button>
                                    <button onClick={() => handleDelete(item.id)} className="text-red-600 hover:bg-red-50 p-1 rounded"><FaTrash /></button>
                                </div>
                            </div>
                            <div className="p-4">
                                <h4 className="font-bold text-gray-800 line-clamp-1" title={item.title}>{item.title}</h4>
                                <p className="text-sm text-gray-500 line-clamp-2 mt-1">{item.designation}</p>
                            </div>
                        </div>
                    ))}
                </div>
